...
function markdownToHtml(markdown) {
  ...
  return markdown
    // Headers
    .replace(/^### (.+)$/gm, '<h3 style="color: #fde047; font-size: 18px; margin: 24px 0 12px 0;">$1</h3>')
    .replace(/^## (.+)$/gm, '<h2 style="color: #fde047; font-size: 22px; margin: 32px 0 16px 0;">$1</h2>')
    // Bold
    .replace(/\*\*(.+?)\*\*/g, '<strong style="color: #f9a8d4;">$1</strong>')
    // Bullet points
    .replace(/^- (.+)$/gm, '<li style="color: #e2e8f0; margin: 8px 0; margin-left: 20px;">$1</li>')
    ...
    .replace(/^(?!<[hul]|<li)(.+)$/gm, '<p style="color: #e2e8f0; line-height: 1.7; margin: 12px 0;">$1</p>')
